...
export async function listStaff(req: AuthRequest, res: Response) {
  ...
  try {
    ...
    const countParams: any[] = [tenantQuery.getTenantId()];
    if (branch_id) {
      countQuery += " AND branch_id = ?";
      countParams.push(branch_id);
    }
    const [countRows] = await tenantQuery.query(countQuery, countParams);
...
export async function createStaff(req: AuthRequest, res: Response) {
  ...
  if (branch_id) {
    const [branchRows] = await tenantQuery.query(
      "SELECT id FROM branches WHERE id = ? AND tenant_id = ?",
      [branch_id, tenantQuery.getTenantId()]
    );
  ...
  try {
    const [result] = await tenantQuery.query(
      ...
      [tenantQuery.getTenantId(), branch_id || null, name, email || null, phone || null, role || "sales", is_active !== undefined ? is_active : 1]
    );
    ...
    const [rows] = await tenantQuery.query(
      ...
      [staffId, tenantQuery.getTenantId()]
    );
...
export async function updateStaff(req: AuthRequest, res: Response) {
  ...
  try {
    await tenantQuery.query(
      ...
      [name, email || null, phone || null, role, branch_id || null, is_active, id, tenantQuery.getTenantId()]
    );
    const [rows] = await tenantQuery.query(
      "SELECT s.*, b.name as branch_name FROM staff s LEFT JOIN branches b ON s.branch_id = b.id WHERE s.id = ? AND s.tenant_id = ?",
...
export async function deleteStaff(req: AuthRequest, res: Response) {
  ...
  try {
    const [result] = await tenantQuery.query("DELETE FROM staff WHERE id = ? AND tenant_id = ?", [id, tenantQuery.getTenantId()]);
